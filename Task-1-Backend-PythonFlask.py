...
def get_json():
    ...
    get_json_authors=r_users.json()
    get_json_posts=r_posts.json()
    return render_template('authors.html',authors=get_json_authors,posts=get_json_posts)
...
def set_cookie():
    ...
    if firstName!='Ritika' or age!='20':
        response.set_cookie('firstName',value='Ritika')
        response.set_cookie('age',value='20')
        return response
    return 'Cookies are already set'
...
def cookie_retrive():
    firstName = request.cookies.get('firstName')
    age = request.cookies.get('age')
    cookie_temp='key: First name : %s' % firstName
    cookie_temp+='<br>key: Age : %s' % age
    return cookie_temp
...
def login():
    ...
    if request.method == 'POST' and request.form['Username'] == 'Ritika' :
